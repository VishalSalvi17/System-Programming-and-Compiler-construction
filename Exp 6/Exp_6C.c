...
int isNonTerminal(char c)//check if character is non terminal_count
{
...
char popFromStack()
{
	char data;
	char emp='N';
	if(isStEmpty())//if stack not empty popFromStack the stack top 
	{
		data=stack[stacktop];
		stacktop-=1;
		return data;
	...
	{	
		...
		else 	//if stacktop is number
		{
			...
			if(action[0]=='s') //if action is shift
			{
				...
				buffidx++;
				pushIntoStack(action[1]);//pushIntoStack action number to stack
